...
func Test_City_Flow(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantError:     ERR_UNKNOWN_DIRECTION,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cityTo, err := city1.GetCityLink(tt.giveDirection)
			require.Equal(t, err, tt.wantError)
			require.Nil(t, cityTo)

			err = city1.SetCityLink(city2, tt.giveDirection)
			require.Equal(t, err, tt.wantError)
			...
			err = city1.RemoveCityLink(city2)
			switch tt.wantError {
			case nil:
				require.Equal(t, err, tt.wantError)
			default:
				require.Equal(t, err, ERR_UNKNOWN_CITY)
			}
